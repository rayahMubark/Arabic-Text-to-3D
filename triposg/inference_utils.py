...
# diso (DiffDMC) اختياري — على ويندوز غالبًا غير متوفر/يبغى بناء
try:
    from diso import DiffDMC  # optional on Windows
    HAS_DISO = True
except Exception:
    HAS_DISO = False
    DiffDMC = None
...
def find_candidates_band(occupancy_grid: torch.Tensor, band_threshold: float, n_limits: int = -1) -> torch.Tensor:
    """
    Returns the coordinates of all voxels where |SDF| < band_threshold (after converting logits->sdf).
    """
    core_grid = occupancy_grid[1:-1, 1:-1, 1:-1]
    # logits -> sdf-ish in [-1,1]
...
def expand_edge_region_fast(edge_coords, grid_size):
    ...
    if grid_size < 512:
        kernel_size = 5
        pooled_tensor = F.max_pool3d(expanded_tensor.unsqueeze(0).unsqueeze(0), kernel_size=kernel_size, stride=1, padding=2).squeeze()
    else:
        kernel_size = 3
        pooled_tensor = F.max_pool3d(expanded_tensor.unsqueeze(0).unsqueeze(0), kernel_size=kernel_size, stride=1, padding=1).squeeze()
...
def parallel_zoom(occupancy_grid, scale_factor):
    return F.interpolate(occupancy_grid.unsqueeze(0).unsqueeze(0), scale_factor=scale_factor).squeeze(0).squeeze(0)


# -------------------------------
# Hierarchical extractor (marching cubes fallback)
# -------------------------------
@torch.no_grad()
def hierarchical_extract_geometry(
    ...
    xyz_samples, grid_size, length = generate_dense_grid_points_gpu(
        bbox_min=bbox_min, bbox_max=bbox_max, octree_depth=dense_octree_depth, indexing="ij"
    )
    ...
    for i in range(hierarchical_octree_depth - dense_octree_depth):
        ...
        grid_size_now = 2 ** curr_octree_depth
        normalize_offset = grid_size_now / 2

        high_res_occupancy = parallel_zoom(grid_logits, 2)

        band_threshold = 1.0
        edge_coords = find_candidates_band(grid_logits, band_threshold)
        expanded_coords = expand_edge_region_fast(edge_coords, grid_size=int(grid_size_now / 2)).to(torch.float16)
    ...
    try:
        ...
        volume = grid_logits.float().cpu().numpy()
        verts, faces, normals, _ = measure.marching_cubes(volume, level=0.0, method="lewiner")
        verts = verts / (2 ** hierarchical_octree_depth) * bbox_size.cpu().numpy() + bbox_min.cpu().numpy()
        mesh_v_f = (verts.astype(np.float32), np.ascontiguousarray(faces))
...
def extract_near_surface_volume_fn(input_tensor: torch.Tensor, alpha: float):
    """
    Args:
        input_tensor: [D, D, D], torch.float16
        alpha: isosurface offset
    Returns:
        mask: [D, D, D], torch.int32
    """
...
# -------------------------------
# Flash extractor with optional diso and fallback to marching_cubes
# -------------------------------
@torch.no_grad()
def flash_extract_geometry(
    ...
    **kwargs,
    ...
    ).permute(0, 2, 4, 1, 3, 5, 6).reshape(-1, mini_grid_size * mini_grid_size * mini_grid_size, 3)
    ...
    for start in range(0, xyz_samples.shape[0], num_batchs):
        ...
        geo_decoder.set_topk(False)  # لا نستخدم top-k حالياً
        logits = vae.decode(batch_latents, queries).sample
        batch_logits.append(logits)

    grid_logits = torch.cat(batch_logits, dim=0).reshape(
        mini_grid_num, mini_grid_num, mini_grid_num, mini_grid_size, mini_grid_size, mini_grid_size
    ).permute(0, 3, 1, 4, 2, 5).contiguous().view((batch_size, grid_size[0], grid_size[1], grid_size[2]))

    # 3) رفع الدقة تدريجياً
    for octree_depth_now in resolutions[1:]:
        ...
        grid_logits_flat = torch.zeros((next_points.shape[1]), dtype=latents.dtype, device=latents.device)
        input_grid = [[], []]
        logits_grid_list = []
        start_num = 0
        sum_num = 0
    ...
    # 4) استخراج السطح — diso إن توفر وإلا marching_cubes
    ...
    try:
        print("final grids shape = ", grid_logits.shape)
        if HAS_DISO and DiffDMC is not None:
            dmc = DiffDMC(dtype=torch.float32).to(grid_logits.device)
            sdf = -grid_logits / (2 ** octree_depth)  # مقياس تقريبي للسالب/الموجب
            ...
            volume = grid_logits.float().cpu().numpy()
            verts, faces, normals, _ = measure.marching_cubes(volume, level=mc_level, method="lewiner")
            verts = verts / (2 ** octree_depth) * bbox_size + bbox_min
            mesh_v_f = (verts.astype(np.float32), np.ascontiguousarray(faces))
    except Exception as e:
        print(e)
        torch.cuda.empty_cache()
